...
shared_ptr<Store> BucketFallbackStore::copy(const std::string &category) {
  BucketFallbackStore *store = new BucketFallbackStore(storeQueue, category,
      multiCategory);
  ...
  for (std::vector<shared_ptr<Store> >::iterator iter = buckets.begin(); iter
      != buckets.end(); ++iter) {
 ...
 */
bool BucketFallbackStore::handleMessages(
