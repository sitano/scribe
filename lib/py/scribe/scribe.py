...
class Client(fb303.FacebookService.Client, Iface):
  ...
  def LogAsync(self, messages):
    ...
    Parameters:
     - messages
    """
    self.send_LogAsync(messages)

  def send_LogAsync(self, messages):
    self._oprot.writeMessageBegin('LogAsync', TMessageType.CALL, self._seqid)
    args = LogAsync_args()
    args.messages = messages
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
...
class Processor(fb303.FacebookService.Processor, Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["LogAsync"] = Processor.process_LogAsync
  ...
  def process_LogAsync(self, seqid, iprot, oprot):
    args = LogAsync_args()
    args.read(iprot)
    iprot.readMessageEnd()
    self._handler.LogAsync(args.messages)
    return
...
class LogAsync_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('LogAsync_args')
    if self.messages is not None:
      oprot.writeFieldBegin('messages', TType.LIST, 1)
      oprot.writeListBegin(TType.STRUCT, len(self.messages))
